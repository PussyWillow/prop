...
export const SettingsModal = ({ isOpen, onClose, onSave, onClear, initialConfig, showToast }: SettingsModalProps) => {
  ...
  const handleClear = () => {
      const confirmation = prompt('This action cannot be undone. To confirm, please type "clear" in the box below.');
      if (confirmation === 'clear') {
        onClear();
        setConfig({ username: '', repo: '', filePath: 'diary.json', token: '' });
        showToast('Sync settings cleared.', 'info');
        onClose();
      } else if (confirmation !== null) { // User typed something else and clicked OK
        showToast('Clear action cancelled. You did not type "clear".', 'error');
      }
      // If confirmation is null, the user clicked "Cancel", so we do nothing.
  };
